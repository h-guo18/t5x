...
def _fp32_softmax(x, dim):
    y = jax.nn.softmax(x, dim=dim, dtype=jnp.float32).type_as(x)
    return y
...
def look_around(x, backward = 1, forward = 0, pad_value = -1, dim = -2):
    dims = (-dim) * (0, 0)
    padded_x = jnp.pad(x, (*dims, backward, forward), value= pad_value)
    if dim == -2:
        t = x.shape[-3]
    ...
    if dim == -1:
        t = x.shape[-2]
        tensors = [padded_x[..., ind:(ind + t), :] for ind in range(forward + backward + 1)]
    return jnp.cat(tensors, dim=dim)
...
def log_matmul_exp(value1, value2, eps=1e-6):
    ...
    # val1: [..., m, c]
    # val2: [..., c, n]
    m1 = jnp.amax(value1, dim=-1, keepdim=True).detach()
    m2 = jnp.amax(value2, dim=-2, keepdim=True).detach()
    new_value_1 = value1 - m1
    new_value_2 = value2 - m2
    return m1 + m2 + jnp.log(jnp.matmul(jnp.exp(new_value_1), jnp.exp(new_value_2)) + eps)
...
def log_avg_exp(
    ...
    if mask is not None:
        mask_value = -jnp.finfo(t.dtype).max
    ...
    out = jnp.log(avg_exp + eps) + max_t - norm
...
class Merger(nn.Module):
    ...
    def forward(self, x, dim=-2, keepdim=False):
        ...
        if self.pooling_fn == 'mean':
            x = jnp.mean(x_prime, dim=dim, keepdim=keepdim)
        elif self.pooling_fn == 'max':
            x = jnp.amax(x_prime, dim=dim, keepdim=keepdim)
        else:
...
def take_along_dim(input, index, dim):
    ...
    return jax.lax.gather(input, dim=dim, index=index)
...
def window_2d_partition(x, window_size, ext_window_size=0, pad_val=0):
    ...
    if ext_window_size > 0:
        ...
        x =jnp.pad(x, [0, 0, ext_window_size, ext_window_size, ext_window_size, ext_window_size], value=pad_val)
...
def hyperm_projection(
    ...
    if diagonal:
        ...
        norm = data_normalizer * jnp.sum(data ** 2, dim=-1).unsqueeze(-2) / 2.0# [n, b, h, 1, lk]
    
    proj_data = math.sqrt(1 / 2.) * (
        jnp.concatenate([
            jnp.exp(data_dash - norm -
                jnp.amax(data_dash, dim=(-1, -2), keepdim=True).detach()),
            jnp.exp(-data_dash - norm -
                jnp.amax(-data_dash, dim=(-1, -2), keepdim=True).detach()),                  
        ], dim=-2) + eps)
...
def prm_projection(
    ...
    if diagonal:
        ...
        norm = data_normalizer * jnp.sum(data ** 2, dim=-1).unsqueeze(-2) / 2.0# [n, b, h, 1, lk]
    if normalize:
        proj_data = jax.nn.softmax(data_dash - norm, dim=-1)  # [n, b, h, l_c, l_k]
    elif return_exp:
        if is_query:
            proj_data = jnp.exp(
                data_dash - norm - jnp.amax(data_dash, dim=-2, keepdim=True).detach()) + eps       
        else:
            proj_data = jnp.exp(
                data_dash - norm - jnp.amax(data_dash, dim=(-1, -2, -3), keepdim=True).detach()) + eps           
    else:
...
def buffered_future_mask(self, tensor):
    ...
    if (
        ...
        self._future_mask = jnp.triu(
            fill_with_neg_inf(jnp.zeros([dim, dim])), 1
        )
